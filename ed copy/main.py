...
def min_distance(origins, destination="Fertility North, Private Hospital, Suite 30, Level 2/60 Shenton Ave, Joondalup WA 6027"):
    ...
    for r in result:
        ...
        smallest = min(smallest, float(
            string.replace(" km", "").replace(",", "")))
    return smallest
...
def placeID(origins):
    ...
    # make sure there are no blank spaces for the URL
    address = ad.replace(' ', '%20')
...
addresses = pd.Series(df.fillna('').values.tolist()).map(
